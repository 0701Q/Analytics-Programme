...
Following closely is the 19-35 Age Group (1,573 cases). This can be attributed to their high mobility and occupational exposure, as they constitute the primary workforce, increasing their likelihood of coming into contact with high-risk environments.
...
# I combine demographic and encounter details for analysis by semi_join().
...
encounter_summary <- covid_enc_master %>%
  filter(ENCOUNTERCLASS %in% c("ambulatory", "emergency", "inpatient", "urgentcare")) %>%
...
#4. Display a summary table showing the relative frequency of each encounter type by age and gender.

kable(
  encounter_summary,
  caption = "Distribution of Encounter Types by Age Group and Gender",
  col.names = c("Age Group", "Gender", "Encounter Type", "Count", "Proportion")
)

#I use kable() to present the results in a professional, report-ready format. The table clearly compares how hospitalisation and encounter types vary across demographics.
...
This question compares characteristics of COVID-affected patients who recovered (any COVID record with a non-missing Stop date) versus those who did not. We analyse how demographics (age, gender, zip/county), symptoms (non-COVID conditions), and timelines (onset-to-recovery duration) differ across outcomes to identify factors associated with recovery.

## Analysis: Recovery vs Non-recovery

```{r}
## Question 4 — Recovery vs Non-recovery 

# Step 1 — Build the COVID cohort at condition level (keep only COVID rows)

covid_cond <- conditions %>%
  filter(str_detect(DESCRIPTION, regex("COVID", ignore_case = TRUE))) %>%
  mutate(
    START_DT = as_date(START),
    STOP_DT  = as_date(STOP)
  ) %>%
  select(PATIENT, START_DT, STOP_DT)

# I isolate COVID diagnoses and standardise START/STOP dates for downstream timeline analysis.

# Step 2 — Derive patient-level recovery outcome and timeline

covid_outcomes <- covid_cond %>%
  group_by(PATIENT) %>%
  summarise(
    first_covid_start = min(START_DT, na.rm = TRUE),
    any_stop          = any(!is.na(STOP_DT)),
    last_covid_stop   = if_else(any_stop, max(STOP_DT, na.rm = TRUE), as_date(NA)),
    duration_days     = if_else(any_stop, as.numeric(last_covid_stop - first_covid_start), NA_real_),
    .groups = "drop"
  ) %>%
  mutate(Recovered = if_else(any_stop, "Yes", "No"))

# A patient is labelled 'Recovered' if any COVID record has a non-missing STOP; duration measures the interval from first COVID to last recorded STOP.

# Step 3 — Attach demographics (reuse Age_Group/GENDER built in Q1)

demo_compact <- covid_master_data %>%
  select(Id, Age, Age_Group, GENDER, RACE, MARITAL, ZIP, COUNTY)

covid_outcomes_demo <- covid_outcomes %>%
  left_join(demo_compact, by = c("PATIENT" = "Id")) %>%
  mutate(
    Age_Group = if_else(is.na(Age_Group), "Unknown", Age_Group),
    Age_Group = factor(Age_Group, levels = c("0-18","19-35","36-50","51+","Unknown"))
  )

# I enrich outcomes with demographics to enable stratified comparisons.

# Step 4 — Recovery vs non-recovery share within Age_Group × Gender  (SINGLE TABLE)

recovery_by_demo <- covid_outcomes_demo %>%
  group_by(Age_Group, GENDER, Recovered) %>%
...
kable(
  recovery_by_demo %>% arrange(Age_Group, GENDER, desc(Recovered)),
  caption   = "Recovery vs Non-recovery (share within Age × Gender)",
  col.names = c("Age Group","Gender","Recovered?","Count","Share")
) 

# The table reports within-group recovery shares to compare patterns across demographics.

# Step 5 — Plot: Recovery vs Non-recovery (stacked 100%) by Age_Group × Gender

ggplot(recovery_by_demo, aes(x = Age_Group, y = Share, fill = Recovered)) +
  geom_col(position = "fill") +
  facet_wrap(~ GENDER) +
  scale_y_continuous(labels = scales::percent) +
  labs(title = "Recovery vs Non-recovery by Age Group and Gender",
       x = "Age Group", y = "Within-group share", fill = "Recovered") +
  theme_minimal()

# A 100%-stacked chart displays the composition of outcomes within each demographic stratum.
```
##Interpretation

The chart shows that nearly all COVID-affected patients in the dataset are recorded as recovered, with recovery proportions approaching 100% across all age and gender groups.
This suggests that the dataset may represent follow-up records where most patients had documented recovery dates.
While the proportion differences are minimal, there is a slight tendency for males in older age groups (51+) to exhibit marginally lower recovery shares than females, reflecting higher vulnerability among elderly male patients.
Overall, age and gender exert limited variation in recovery likelihood within this synthetic cohort.

```{r}
# Step 6 — Plot: Recovery duration distribution (Recovered only)

covid_outcomes_demo %>%
  filter(Recovered == "Yes", !is.na(duration_days)) %>%
  ggplot(aes(x = Age_Group, y = duration_days)) +
  geom_boxplot() +
  labs(title = "Recovery Duration by Age Group (Recovered only)",
       x = "Age Group", y = "Days from first COVID to last recovery date") +
  theme_minimal()

# Boxplots (Recovered only) summarise recovery timelines; medians and IQR highlight age gradients.
```
##Interpretation

The boxplot displays recovery timelines (in days) among recovered patients.
Median recovery durations fall between 20–25 days across all age groups, indicating relatively consistent recovery times.

However, the older groups (36–50 and 51+) show a wider interquartile range, suggesting greater variability and potentially slower recovery for some individuals.
Younger cohorts (0–18 and 19–35) show slightly shorter and more compact duration ranges, reflecting faster and more uniform recovery patterns due to stronger immune responses and fewer comorbidities.

```{r}
# Step 7 — Non-COVID symptoms by outcome (Top-5 each outcome; bar chart only)

# Keep NON-COVID conditions but ONLY for patients in the COVID cohort

covid_comorbidities <- conditions %>%
  semi_join(covid_outcomes %>% select(PATIENT), by = "PATIENT") %>%              # keep COVID cohort patients
  filter(!str_detect(DESCRIPTION, regex("COVID", ignore_case = TRUE))) %>%       # exclude COVID rows
# Clean and standardise the condition text
  mutate(DESCRIPTION = DESCRIPTION %>% str_squish() %>% str_to_title()) %>%
  filter(!is.na(DESCRIPTION) & DESCRIPTION != "") %>%
  select(PATIENT, DESCRIPTION)

# Attach recovery status to each patient-condition pair

symptoms_by_outcome <- covid_comorbidities %>%
  inner_join(covid_outcomes %>% select(PATIENT, Recovered), by = "PATIENT") %>%
  distinct(Recovered, PATIENT, DESCRIPTION) %>%     # unique patient-condition pairs
  count(Recovered, DESCRIPTION, sort = TRUE)

# Select the Top-5 symptoms separately for each outcome (Yes/No)

top5_symptoms_each <- symptoms_by_outcome %>%
  group_by(Recovered) %>%
  slice_head(n = 5) %>%
  ungroup()

# Visualise Top-5 lists with a single horizontal bar chart

ggplot(top5_symptoms_each, aes(x = reorder(DESCRIPTION, n), y = n, fill = Recovered)) +
  geom_bar(stat = "identity") +
  coord_flip() +
  labs(title = "Top-5 Non-COVID Symptoms by Recovery Outcome (Unique Patients)",
       x = "Symptom / Condition", y = "Unique Patients", fill = "Recovered") +
  theme_minimal()

# This bar chart contrasts the most prevalent non-COVID symptoms among recovered vs non-recovered patients.

```
##Interpretation

The five most common non-COVID symptoms among recovered patients are Fever, Cough, Loss of Taste, Fatigue, and Obesity (BMI 30+).
These align closely with the known clinical presentation of COVID-19, confirming that recovered patients often experienced the core acute symptoms but later resolved.
Because nearly all patients were classified as recovered, the chart only shows the “Recovered” group — reinforcing that most symptom data belong to resolved cases.
This indicates effective management or follow-up among patients recorded in the dataset.
